...
const audioBtn = document.querySelector('.btn__sound');
const audioIcon = document.querySelector('.btn__sound i');
...
let savedMuted = localStorage.getItem('sc2_audio_muted');
audio.muted = savedMuted === null ? true : savedMuted === 'true';
updateIcon();

if (!audio.muted) {
  audio.play().catch(()=> {
    // автоплей заблокирован — оставляем включённым в настройках, звук начнёт после явного клика
  });
}

audioBtn.addEventListener('click', () => {
  audio.muted = !audio.muted;
  localStorage.setItem('sc2_audio_muted', String(audio.muted));
  updateIcon();
  if (!audio.muted) {
    audio.play().catch(()=>{/* проигрывание будет доступно при следующем явном действии */});
  }
});
